function printmmfplot(component,s,d,cc,varargin)
...
for i=2:numel(vars)
    ...
    vlines = str2dat(ser.userdata.EndDate,'dateformat','dd-mmm-yyyy');
    shortsource = ser.userdata.ShortSource;
    longsource = ser.userdata.LongSource;
    SeasAdj = ['Data SA using X12'];
    
    c(5).test = sprintf('date > %s && isempty(strfind(mark,''Delta''))',vlines);
    c(5).format = '\color{blue}';
    
    x.graph([tmp_comment ['DataEndDate: ' enddate]],'legend',true);
    
    
    x.series('Level',ser);
    x.vline('Forecast',vlines);
    ...
    if  isfield(opt,'vintage')
        ...
        h = load(['Assumptions' filesep cc '_assumptions_' dateselect]);
...
x.publish([cc '_' component 'Forecast_' reportdate '.pdf']);
...
%Make forecast reports directory
if ~exist('Reports\ForecastReports','dir')
    mkdir('Reports\ForecastReports');
end

%Make component directory
if ~exist(['Reports\ForecastReports\' component],'dir')
    mkdir(['Reports\ForecastReports\' component]);
end

movefile([cc '_' component 'Forecast_' reportdate '.pdf'],[pwd '\Reports\ForecastReports\' component]);
